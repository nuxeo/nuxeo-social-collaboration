...
/**
 * This class gives method to make the document public or private into the constrains of social workspace. The document
 * must be part of a social workspace and be a social document. A document is public if it's published into the public
 * section. Except for an article render a social document visible only for the community is to publish the document
 * into the private section. For an article this is just to have no proxy into the public section. The source document
 * himself will be the document exposed. In the documentation about method of this class, when we reference to a social
 * document, we implicitly talking about the social document handle by the instance of the class set into the
 * constructor.
 *
...
public interface SocialDocument {
    ...
    /**
     * Returns {@code true} if this document is inside a {@code SocialWorkspace} , {@code false} otherwise.
     */
    ...
    /**
     * Returns {@code true} if this document is restricted to a {@code SocialWorkspace} members, {@code false}
     * otherwise.
     */
