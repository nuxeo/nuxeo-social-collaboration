...
public class SocialWorkspaceDefaultSpaceCreator extends DefaultDashboardSpaceCreator {
    ...
    protected void initializeGadgets(Space space, CoreSession session, Locale locale) throws ClientException {
        WebContentHelper.createOpenSocialGadget(space, session, locale, "usersocialworkspaces", 0, 0, 0);
        WebContentHelper.createOpenSocialGadget(space, session, locale, "news", 0, 0, 1);
        WebContentHelper.createOpenSocialGadget(space, session, locale, "publicsocialworkspaces", 0, 0, 2);
        WebContentHelper.createOpenSocialGadget(space, session, locale, "userworkspaces", 0, 0, 3);
        WebContentHelper.createOpenSocialGadget(space, session, locale, "userdocuments", 0, 0, 4);

        WebContentHelper.createOpenSocialGadget(space, session, locale, "minimessages", 0, 1, 0);
        WebContentHelper.createOpenSocialGadget(space, session, locale, "useractivitystream", 0, 1, 1);
        WebContentHelper.createOpenSocialGadget(space, session, locale, "tasks", 0, 1, 3);
    }
}
