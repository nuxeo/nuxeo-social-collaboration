...
import static org.nuxeo.ecm.social.workspace.SocialConstants.GLOBAL_DASHBOARD_SPACE_NAME;
 ...
 * @since 5.4.3
 */
public class GlobalDashboardSpaceProvider extends AbstractSpaceProvider {

    private static final Log log = LogFactory.getLog(GlobalDashboardSpaceProvider.class);
    ...
    protected Space getOrCreateSpace(CoreSession session,
        ...
        DocumentModel socialWorkspaceContainer = getSocialWorkspaceService().getOrCreateSocialWorkspaceContainer(
                session);
        DocumentRef globalDashboardSpaceRef = new PathRef(socialWorkspaceContainer.getPathAsString(), SocialConstants.GLOBAL_DASHBOARD_SPACE_NAME);
        if ( session.exists(globalDashboardSpaceRef)) {
            ...
            DocumentModel globalDashboardSpace = getOrCreateGlobalDashboardSpace(session, parameters);
            return globalDashboardSpace.getAdapter(Space.class);
        }
    ...
    protected DocumentModel getOrCreateGlobalDashboardSpace(
        ...
        GlobalDashBoardSpaceCreator creator = new GlobalDashBoardSpaceCreator(
                session, parameters);
    ...
    class GlobalDashBoardSpaceCreator extends AbstractDashboardSpaceCreator {

        DocumentRef globalDashboardSpaceRef;

        protected GlobalDashBoardSpaceCreator(CoreSession session,
        ...
        public void run() throws ClientException {
            ...
            DocumentModel globalDashboardSpace = session.createDocumentModel(
                    ...
                    GLOBAL_DASHBOARD_SPACE_NAME, SPACE_DOCUMENT_TYPE);
